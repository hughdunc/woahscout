...
@export var blue_points_box: Control
@export var blue_plus: Button
@export var blue_count: Label
@export var blue_minus: Button
...
func get_auto_switch_name(menu: MatchMenu) -> String:
	match menu:
		...
		MatchMenu.WHO_GOT_FIRST:
			return "WHO SHIFT 1"
		MatchMenu.BLUE_SHIFT_1:
			return "SHIFT 2" if first_shift == "red" else "SHIFT 1"
		MatchMenu.RED_SHIFT_1:
			return "SHIFT 1" if first_shift == "red" else "SHIFT 2"
		MatchMenu.BLUE_SHIFT_2:
			return "SHIFT 4" if first_shift == "red" else "SHIFT 3"
		MatchMenu.RED_SHIFT_2:
...
func load_menu(menu: MatchMenu):
	...
	var c = menu_configs_blue[menu]
	var fs_no = first_shift == "" and menu in [MatchMenu.BLUE_SHIFT_1, MatchMenu.RED_SHIFT_1, MatchMenu.RED_SHIFT_2, MatchMenu.BLUE_SHIFT_2]
	if fs_no:
		for n in c["visible"]:
			n.visible = false
		for n in c["hidden"]:
...
func _ready():
	team_num.text = str(Global.team_scouting)
	
	menu_configs_blue = {
		MatchMenu.AUTO: {
			"name": "AUTONOMOUS",
			"visible": [during_game, auto_box, neutral_plus, neutral_count, neutral_points_box, blue_points_box, neutral_minus, blue_plus, blue_count, blue_minus, no_auto, climb_auto],
			"hidden": [post_game, opponent_box, blue_gets_first, red_gets_first, blue_got_first, who_got_label, red_got_first],
		},
		MatchMenu.TRANSITION: {
			"name": "TRANSITION",
			"visible": [during_game, opponent_box, blue_gets_first, red_gets_first, defend_button, neutral_plus, neutral_count, neutral_points_box, blue_points_box, neutral_minus, blue_plus, blue_count, blue_minus],
		...
		MatchMenu.WHO_GOT_FIRST: {
			...
			"hidden": [post_game, opponent_box, defend_button, blue_gets_first, red_gets_first, neutral_plus, neutral_count, neutral_points_box, blue_points_box, neutral_minus, blue_plus, blue_count, blue_minus, climb_option, auto_box],
		},
		MatchMenu.BLUE_SHIFT_1: {
			"name": "BLUE SHIFT 1",
			"visible": [during_game, opponent_box, defend_button, neutral_plus, neutral_count, neutral_points_box, blue_points_box, neutral_minus, blue_plus, blue_count, blue_minus],
			"hidden": [post_game, climb_option, auto_box, blue_gets_first, red_gets_first, blue_got_first, who_got_label, red_got_first],
		},
		MatchMenu.RED_SHIFT_1: {
			"name": "RED SHIFT 1",
			"visible": [during_game, opponent_box, defend_button, neutral_plus, neutral_count, neutral_points_box, neutral_minus, blue_plus, blue_count, blue_minus],
			"hidden": [post_game, climb_option, auto_box, blue_gets_first, red_gets_first, blue_got_first, who_got_label, red_got_first, blue_points_box],
		},
		MatchMenu.BLUE_SHIFT_2: {
			"name": "BLUE SHIFT 2",
			"visible": [during_game, opponent_box, defend_button, neutral_plus, neutral_count, neutral_points_box, blue_points_box, neutral_minus, blue_plus, blue_count, blue_minus],
		...
		MatchMenu.ENDGAME: {
			"name": "ENDGAME",
			"visible": [during_game, opponent_box, climb_option, defend_button, neutral_plus, neutral_count, neutral_points_box, blue_points_box, neutral_minus, blue_plus, blue_count, blue_minus],
			"hidden": [post_game, auto_box, blue_got_first, blue_gets_first, red_gets_first, who_got_label, red_got_first],
		},
		MatchMenu.POST_GAME: {
			"name": "POST GAME",
			"visible": [post_game],
			"hidden": [during_game, blue_gets_first, opponent_box, defend_button, climb_option, auto_box, no_auto, climb_auto, blue_got_first, neutral_points_box, neutral_plus, neutral_count, neutral_minus, who_got_label, red_gets_first, blue_points_box, blue_plus, blue_count, blue_minus, red_got_first],
		}
	}
	...
	# Connect to blue points controls
	blue_plus.pressed.connect(func(): change_points("scored", 5))
	blue_minus.pressed.connect(func(): change_points("scored", -5))
...
func update_points():
	if blue_count.is_visible_in_tree():
		blue_count.text = str(stats[current_menu]["scored"])
	...
	if defend_button.is_visible_in_tree():
		defend_button.text = "HOLD WHILE DEFENDING " + format_time(ceil(stats[current_menu]["defended"]))
